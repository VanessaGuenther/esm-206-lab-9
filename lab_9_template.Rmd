...
# need continous variable for linear Regrssion

# a. Saturated model:  # Price dependent variable # Price per square foot ( derived from each other - so that will be no uncertainity- increases model fit artificially)
...
# a. Updated model with variables City, SqFt, and Status: # intercept is city grande 
...
homes_sub$Status <- fct_relevel(homes_sub$Status, "Regular")  # take Regular as Reference Level 
...
# a. Model diagnostics: # looks relatively heterosydastic ( smaller data set) Grab 188 - sold for less then expected 
 plot(homes_lm3)
```
...
# a. AIC values for each model ( only comparing models )

full_aic <- AIC(homes_lm1)
final_aic <- AIC(homes_lm3)

#would pick 1st one 10699 (pick) comapred to 11148

# Answer: which would you pick? 
#BUT DONT becuase of squarefeatprice
```
...
# a. Prepare a nice regression table: (open HTML doc in word)
